...
// Mesmo critério do portal web: serviços cuja data já passou (ou o mês
// previsto já passou, se ainda não confirmada) — mais recentes primeiro.
export default function RealizadosScreen() {
  ...
  return (
    <>
      <Stack.Screen options={{ title: 'Realizados' }} />
      <FlatList
        style={{ backgroundColor: colors.bg }}
        contentContainerStyle={styles.list}
        data={schedules}
        keyExtractor={(item) => item.id}
        ListEmptyComponent={<Text style={styles.empty}>Nenhum serviço realizado ainda.</Text>}
        renderItem={({ item }) => {
