// Script avulso: sobe/atualiza só o CustodyFieldTemplate de Óleo no banco já
// rodando, sem reexecutar o seed inteiro (que teria efeitos colaterais em
// dados de teste não-idempotentes — schedules/samples). Roda com:
//   npx ts-node scripts/seed-oleo-custody-template.ts
// Mesma definição usada em prisma/seed.ts (OLEO_CUSTODY_TEMPLATE) — mantidas
// em sincronia manualmente, já que seed.ts executa main() como efeito
// colateral da importação e não dá pra importar o objeto de lá direto.
...
async function main() {
  ...
  console.log('Template de Óleo salvo:', template.id);
}
